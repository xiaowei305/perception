...
class Semantic_KITTI_Utils():
    def __init__(self, root, subset = 'all'):
        ...
        self.R, self.T = self.calib_velo2cam(os.path.join(base_path, 'config', 'calib_velo_to_cam.txt'))
        self.P = self.calib_cam2cam(os.path.join(base_path, 'config', 'calib_cam_to_cam.txt') ,mode="02")
        self.RT = np.concatenate((self.R, self.T), axis=1)

        self.sem_cfg = yaml.load(open(os.path.join(base_path, 'config', 'semantic-kitti.yaml'), 'r'),
