...
class Conv(Trainable):
    def __init__(self, name, kernel, inc, outc):
        ...
        self.inc = inc
        self.outc = outc
        # 使用msra方法初始化权重， 注意N的计算（k * k * input_channels)
    ...
    def forward(self, x):
        ...
        # 使用两重循环实现卷积, 每次移动一个像素，使用矩阵乘法(dot)来计算输出
    ...
    def backward(self, diff):
        ...
        # 计算loss相对于weight的梯度（用下一层梯度作为卷积核，卷输入）
        ...
        # 计算loss相对于bias的梯度，将下一层梯度求和即可
        bias_diff = np.sum(diff, axis=(0, 1, 2))

        # 计算loss相对于输入的梯度（用反转的卷积核，卷下一层的梯度）
        pad = k - 1
...
class Pooling():
    def forward(self, x):
        ...
        # 哪个像素被取到了，对应mask就是1，否则是0，用来记录max pooling是从哪个像素取的
    ...
    def backward(self, diff):
        ...
        # max pooling的反向传播，就是用mask与下一层梯度相乘
